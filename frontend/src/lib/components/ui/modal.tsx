  ...
  return (
    <ModalContext.Provider value={{ onClose }}>
      <dialog
        ...
        onClose={onClose}
        className={cn(
          "fixed top-1/2 left-1/2 w-full max-w-xl min-w-xs -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-md border border-zinc-800 sm:min-w-lg",
          className,
        )}
      >
        {children}
...
const Footer = ({ children, className }: HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
      "flex justify-end border-t border-t-zinc-800 bg-zinc-950 p-6",
      className,
    )}
  >
    {children}
