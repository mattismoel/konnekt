...
const Footer = () => {
	...
	return (
		<footer className="px-auto border-t border-t-zinc-900 bg-zinc-950 pt-8 pb-6">
			...
			<span className="text-text/50 text-center text-xs line-clamp-1">&copy; {currentYear} Foreningen Konnekt</span>
		</footer >
	)
}
...
const ContactEntry = ({ value, type }: { type: ContactType, value: string }) => (
	<a
...
const NavList = ({ links }: { links: Map<NavPath, string> }) => (
	<ul className="text-text/50">
		{Array.from(links).map(([path, name]) => (
			<li className="hover:underline"><Link to={path}>{name}</Link></li>)
		)}
	</ul>
)
