...
const Footer = () => {
  ...
  return (
    <footer className="border-t border-t-zinc-900 bg-zinc-950 px-auto pt-8 pb-6">
      <div className="mb-8 grid w-full grid-cols-1 gap-8 sm:grid-cols-2">
        <div className="flex-1">
          <span className="mb-2 inline-block align-top font-heading leading-none font-bold">
            Find rundt
          </span>
        ...
        <div className="flex flex-1 flex-col items-start gap-4 sm:items-end">
          ...
          <address className="flex flex-col items-start gap-2 text-text/50 not-italic sm:items-end">
      ...
      <span className="line-clamp-1 text-center text-xs text-text/50">
...
const SocialMediaList = ({ socialMap }: { socialMap: Map<string, Src> }) => (
  <ul className="flex items-center gap-4 text-xl text-text/50">
...
const ContactEntry = ({
  ...
  <a href={`${contactTypeMap.get(type)}:${value}`} className="hover:underline">
    {value}
  </a>
);
