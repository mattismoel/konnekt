...
const Toast = ({ toast, onDelete }: Props) => {
  ...
  return (
    <div
      className={cn(
        "pointer-events-none relative flex min-w-sm animate-fade-in flex-col rounded-sm border p-2",
        {
    ...
    >
      ...
      >
        ...
        <span
          className={cn({
            "font-medium": toast.message && toast.message !== "",
          })}
        >
          {toast.title}
        </span>
        <button
          ...
          onClick={onDelete}
          className={cn(
            "pointer-events-auto absolute top-2 right-2 rounded-full p-1",
            {
              "border-blue-900 bg-blue-950": toast.severity === "info",
              "border-red-900 bg-red-950": toast.severity === "error",
              "border-yellow-900 bg-yellow-950": toast.severity === "warning",
