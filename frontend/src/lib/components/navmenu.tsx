...
const BackgroundOverlay = ({ expanded }: OverlayProps) => (
  <div
    className={cn(
      "fixed z-50 h-svh w-full bg-black opacity-0 transition-opacity",
      {
...
const CloseButton = forwardRef<HTMLButtonElement, CloseButtonProps>(
  ({ expanded, ...rest }, ref) => {
    return (
      ...
      >
        <BiCaretUp
          className={cn("rotate-0 transition-transform duration-400", {
            "rotate-180": expanded,
          })}
        />
      </button>
      ...
      return (
        <li className="cursor-pointer">
          <a
            ref={ref}
            {...rest}
            className={cn("text-3xl text-text-light-muted", {
              "font-bold text-heading": isActive,
            })}
          >
            {children}
          </a>
