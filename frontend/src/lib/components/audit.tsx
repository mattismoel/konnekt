...
const Audit = ({ updatedByMember, updatedAt }: Props) => {
	...
	return (
		<div className="flex items-center gap-4">
			<img src={updatedByMember.profilePictureUrl} className="h-6 aspect-square rounded-full" />
			<span className="text-text/75">
				Redigeret af&nbsp;
				{member?.id === updatedByMember.id
					? "dig"
					: (
						<Link to="/admin/members/$memberId" params={{ memberId: updatedByMember.id.toString() }} className="hover:underline">
