...
import type { Concert, Event } from "./features/event";
...
const formatGoogleCalendarDates = (first: Concert, last: Concert) => {
	const fromDateStr = format(first.fromDate, GOOGLE_CALENDAR_DATETIME_FORMAT);
	const toDateStr = format(last.toDate, GOOGLE_CALENDAR_DATETIME_FORMAT);
